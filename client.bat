...
rem Runs the client Java application.  You must build the application before this will work, use "mvn package" to do so.

rem Usage: client <port> [cipher suite name]*
rem Where:
rem 	<port> is the port to connect to a running server on this host (see Server.bat)
rem 	[cipher suite name] is the name of zero or more cipher suites to force the client to use.
rem
rem See the source code Client.java for more informaton.
